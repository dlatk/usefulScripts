...
def parse_args():
    ...
    p.add_argument("--min_chars_to_count", type=int, default=24,
...
def main():
    ...
    sql = f"""
        SELECT
            ...
            SUBSTRING(m.message, 1, 30)                  AS excerpt,
    ...
    def flush_block():
        ...
        if not (limit_groups and total_dupe_groups > limit_groups):
            ...
            excerpt_disp = (excerpt + "…") if len(excerpt) == 30 else excerpt
    ...
    try:
        cur.execute(sql, (min_chars, args.min_groups, min_chars))
        for row in cur:
            # row: (msg_md5, excerpt, full_message, mlen, message_id, group_id)
            msg_md5, excerpt, _full_message, mlen, message_id, group_id = row
